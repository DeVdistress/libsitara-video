...
static vector<PixelFormat> v4l2_get_formats(int fd, uint32_t buf_type)
{
	...
	{
#if (0) //DeVdistress was changed for good working vpe
...
static void v4l2_set_format(int fd, PixelFormat fmt, uint32_t width, uint32_t height, uint32_t buf_type, uint8_t use_deinterlace = 0)
{
	int r;
	...
	if (mplane) {
		...
		switch (use_deinterlace)
		{
		case 1:
			v4lfmt.fmt.pix_mp.field = V4L2_FIELD_ALTERNATE;
			v4lfmt.fmt.pix_mp.colorspace = V4L2_COLORSPACE_SMPTE170M;
			break;
		case 2:
			v4lfmt.fmt.pix_mp.field = V4L2_FIELD_SEQ_TB;
			break;
		case 0:
		default:
			v4lfmt.fmt.pix_mp.field = V4L2_FIELD_ANY;
			break;
		}
...
static void v4l2_queue_dmabuf(int fd, uint32_t index, Framebuffer* fb, uint32_t buf_type, const uint32_t *field = nullptr)
{
	v4l2_buffer buf { };
	buf.type = buf_type;
...
static uint32_t v4l2_dequeue(int fd, uint32_t buf_type, uint32_t *field = nullptr)
{
	v4l2_buffer buf { };
	buf.type = buf_type;
	buf.memory = V4L2_MEMORY_DMABUF;
...
VideoStreamer* VideoDevice::get_capture_streamer(bool for_vpe)
{
	ASSERT(m_has_capture);

	VideoStreamer::StreamerType type;

	if(!m_capture_streamer)
	{
		if		(!for_vpe)
			type = m_has_mplane_capture ? VideoStreamer::StreamerType::CaptureMulti : VideoStreamer::StreamerType::CaptureSingle;
		else if (for_vpe)
			type = m_has_mplane_output ? VideoStreamer::StreamerType::OutputMulti : VideoStreamer::StreamerType::OutputSingle;
...
VideoStreamer* VideoDevice::get_output_streamer(bool for_vpe)
{
	ASSERT(m_has_output);
...
void VideoStreamer::set_format(PixelFormat fmt, uint32_t width, uint32_t height, uint8_t use_deinterlace)
{
	v4l2_set_format(m_fd, fmt, width, height, get_buf_type(m_type), use_deinterlace);
}
...
void VideoStreamer::queue(Framebuffer* fb, const uint32_t *field, const uint32_t *buf_type)
{
	uint32_t idx;
	...
	if(buf_type == nullptr)
		v4l2_queue_dmabuf(m_fd, idx, fb, get_buf_type(m_type), field);
	else
		v4l2_queue_dmabuf(m_fd, idx, fb, *buf_type, field);
}

Framebuffer* VideoStreamer::dequeue(uint32_t *field, uint32_t *idx_)
{
	uint32_t idx = v4l2_dequeue(m_fd, get_buf_type(m_type), field);
